...
% Original file name: "Fig_1h_24p9mrad_Themis_runPtycho.m"
...
cSAXS_matlab_path = '/home/fs01/cl2696/workspace/fold_slice';
ptycho_matlab_path = '/home/fs01/cl2696/workspace/fold_slice/ptycho';
% cSAXS_matlab_path = 'H:/workspace/fold_slice';
% ptycho_matlab_path = 'H:/workspace/fold_slice/ptycho';
...
scan_number = 10; %Ptychoshelves needs
...
errmetric = 'ptyrad';            % optimization likelihood - poisson, L1
...
dp = single(dp);
fprintf('Original dp min = %.4f, mean = %.4f, max = %.4f\n', min(dp(:)), mean(dp(:)), max(dp(:)));
dp = dp - min(dp);
dp = dp ./ max(squeeze(mean(mean(dp, 3), 4)), [], 'all');
fprintf('Processed dp min = %.4f, mean = %.4f, max = %.4f\n', min(dp(:)), mean(dp(:)), max(dp(:)));
...
for grouping = groupings
    for Nprobe = Nprobes
        fprintf('Current grouping: %d, Current Nprobe: %d\n', grouping, Nprobe);
        ...
        if Nlayers>1
            ...
            eng. layer4pos = layer4pos;                  % Added by ZC. speficy which layer is used for position correction ; if empty, then default, ceil(Nlayers/2)
            ...
            eng.save_images ={'obj_ph_stack','obj_ph_sum'}; %{'obj_ph','obj_mag','obj_ph_stack','obj_ph_sum','probe','probe_mag'};
